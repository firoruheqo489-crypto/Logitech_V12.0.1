...
type ColumnDef = {
  ...
  width: string
  editable?: boolean
  type?: "number" | "status" | "vector" | "poka"
  sticky?: StickyTier
}

interface PfmeaDataGridProps {
  data: PfmeaRow[]
  selectedRowId: string | null
  onSelectRow: (id: string) => void
  onVisibleRowsChange?: (rows: PfmeaRow[]) => void
  onUpdateRow: (
    id: string,
    field: EditableField,
...
function getStatusLabel(status: PfmeaStatus) {
  if (status === "closed") return "已闭环"
  if (status === "testing") return "验证中"
  return "待处理"
}

function getNextStatus(status: PfmeaStatus): PfmeaStatus {
...
function getBodyCellClass(
  ...
  const alignClass =
    column.align === "center" ? "px-2 text-center" : "pl-4 pr-3 text-left"
  const baseClass = [
    "align-top whitespace-nowrap py-1.5 text-[11px]",
    alignClass,
    column.width,
...
function getSequenceBodyCellClass(
  ...
  return [
    "sticky left-0 z-10",
    sequenceColumnWidth,
    "align-top border-b border-white/[0.04] px-2 py-1.5 text-center",
    isCriticalPfmeaRisk(row.sev, row.rpn)
      ? "border-l-2 border-l-red-500"
      : "border-l-2 border-l-transparent",
    isSelected
...
function getRpnBadge(row: Pick<PfmeaRow, "sev" | "rpn">) {
...
function getStatusBadgeClass(status: PfmeaStatus, row: Pick<PfmeaRow, "sev" | "rpn">) {
  const isCriticalOpen = status === "pending" && isCriticalPfmeaRisk(row.sev, row.rpn)
...
export function PfmeaDataGrid({
  ...
  const [sortKey, setSortKey] = useState<keyof PfmeaRow>("rpn")
  const [sortDir, setSortDir] = useState<"asc" | "desc">("desc")

  const toggleSort = useCallback((key: keyof PfmeaRow) => {
    setSortKey((prevKey) => {
      if (prevKey === key) {
  ...
  const handleCellChange = useCallback(
    (rowId: string, field: EditableField, value: string | number) => {
      ...
      if (field === "status") {
        onUpdateRow(rowId, field, value as PfmeaStatus)
        return
      }
  ...
  const renderCell = useCallback(
    (row: PfmeaRow, column: ColumnDef) => {
      if (column.key === "status") {
        return (
          <button
            type="button"
            onClick={() => handleCellChange(row.id, "status", getNextStatus(row.status))}
      ...
      const value = row[column.key as keyof PfmeaRow]
      ...
      if (column.editable && column.type === "number") {
        return (
          <input
            ...
            onChange={(event) =>
              handleCellChange(row.id, column.key as EditableField, event.target.value)
            }
      ...
      return <span className="block truncate text-[11px] text-zinc-300">{String(value)}</span>
    },
    [handleCellChange]
  )

  const getRowClassName = (row: PfmeaRow) => {
    const isSelected = row.id === selectedRowId
    const isCritical = isCriticalPfmeaRisk(row.sev, row.rpn)
          ...
          <tr>
            ...
            {columns.map((column) => (
              <th key={column.key} className={getHeaderCellClass(column)}>
                <button
                  type="button"
                  onClick={() => toggleSort(column.key as keyof PfmeaRow)}
        ...
        <tbody>
          ...
          {sortedData.map((row, rowIndex) => (
            ...
            >
              ...
              >
                <span className="font-mono text-[11px] text-zinc-400">{rowIndex + 1}</span>
              </td>
              {columns.map((column) => (
                <td
                  key={column.key}
