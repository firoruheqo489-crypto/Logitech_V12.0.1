...
type PfmeaHeaderFieldKey = "projectName" | "partNumber" | "owner" | "reviewDate"

export type PfmeaHeaderFields = Record<PfmeaHeaderFieldKey, string>

const headerItems: Array<{
  key: PfmeaHeaderFieldKey
  label: string
  placeholder: string
  type?: "text" | "date"
}> = [
  { key: "projectName", label: "项目名称", placeholder: "输入 PFMEA 项目名称" },
  { key: "partNumber", label: "产品/机种", placeholder: "输入产品型号或产线代号" },
  { key: "owner", label: "责任部门", placeholder: "输入 PE / ME / QE / TE" },
  { key: "reviewDate", label: "评审日期", placeholder: "", type: "date" },
]

interface PfmeaContextHeaderProps {
  fields: PfmeaHeaderFields
  onChange: (field: PfmeaHeaderFieldKey, value: string) => void
}
...
export function PfmeaContextHeader({
  fields,
  onChange,
}: PfmeaContextHeaderProps) {
  return (
