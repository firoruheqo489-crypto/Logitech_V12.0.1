...
/* Phase 15 鈥?骞跺彂鑹茶氨鏄犲皠寮曟搸 (Concurrent Color Palette) */
...
/* ========================== Phase 20 鈥?Entity Slider (GanttBar) ========================== */
...
/**
 * Lithography Glass Engine 鈥?3-layer 鐗╃悊鏉愯川娓叉煋
 *   Layer 1: Frosted Glass Shell (纾ㄧ爞鐜荤拑妲斤紝鎵胯浇鎬诲伐鏈?
 *   Layer 2: Plasma Fill (鑳介噺娉ㄥ叆灞傦紝鎸夎繘搴﹀～鍏?
 *   Layer 3: Holographic Text (鎮诞鏂囨湰)
 */
...
function deriveBarStatus(
  ...
  // 閫炬湡浠诲姟涓嶅啀寮哄埗绾㈣壊 鈥?缁熶竴璧版鑹茬郴锛屽彧鏈夊欢鏈熸姤澶囨鎵嶅厑璁哥孩
...
export function GanttSkeleton({ initialComponents, initialMilestones = [], dayWidth: dayWidthFallback = 28 }: GanttSkeletonProps) {
  ...
  // Phase 9 鈥?Milestone deletion state
  ...
  // Phase 8 鈥?Milestone spawner (ADMIN only) 鈥?鎵撳紑缁熶竴寮圭獥
  const handleAddMilestone = useCallback(() => {
    if (role !== "ADMIN") return
    setPromptState({ kind: "milestone" })
  }, [role])

  // Phase 13 鈥?Add component group 鈥?鎵撳紑缁熶竴寮圭獥
  const handleAddComponentGroup = useCallback(() => {
    setPromptState({ kind: "component" })
  }, [])

  // Phase 21 鈥?Delete component group
  ...
  // Phase 23 - 跟踪右侧面板宽度，用于计算动态 dayWidth
    ...
    <div className="flex flex-col w-full h-screen bg-[#0B0F19] text-slate-100">
      ...
      <div className="flex flex-1 min-h-0 border-t border-slate-800/30 relative">
        ...
        <div className="basis-[236px] min-w-[176px] max-w-[248px] shrink-0 border-r border-slate-800/40 flex flex-col bg-[#111827]">
          ...
          {/* Section header (浠呮枃鏈弿杩帮紝涓庡彸渚?ruler 瀵归綈) */}
          {/* 楂樺害浠?GANTT_ROW_H.RULER 鍙栧€硷紝top 鍋忕Щ涓?CONTROL_BAR 楂樺害 */}
          <div
            className="border-b border-slate-800/50 sticky bg-[#0f1729] flex items-center px-3 z-10"
            style={{ height: GANTT_ROW_H.RULER, top: GANTT_ROW_H.CONTROL_BAR }}
          >
            <button
              type="button"
              onClick={openMonthPicker}
              className="flex items-center gap-2 rounded-md px-2 py-1 text-sky-300 whitespace-nowrap hover:bg-sky-500/10 hover:text-sky-200 transition-all"
              aria-label="更换月份"
              title="点击更换月份"
            >
              ...
              <span className="text-[14px] font-normal tabular-nums tracking-wide whitespace-nowrap">
                {monthLabel}
              </span>
            </button>
            <input
        ...
        {/* --------------------- Right 鈥?Timeline Tracks (Phase 23.2: dayWidth 鍔ㄦ€佸～鍏呬娇鏈湀鍒氬ソ閾烘弧) --------------------- */}
        <div ref={rightScrollRef} className="min-w-0 flex-1 overflow-auto relative bg-[#0B0F19] [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
          {/* Phase 18 鈥?Milestone Strip锛氱嫭绔嬩竴琛屻€佺揣璐?top-0銆佷綅浜?ruler 涓婃柟 */}
          {/* 楂樺害浠?GANTT_ROW_H.CONTROL_BAR 鍙栧€硷紝涓庡乏渚ф帶鍒舵爮鐗╃悊閿佹 */}
          <div
          ...
          >
            ...
            {components.map((group, idx) => {
              ...
              return isMacro ? (
                ...
                /* MICRO: 灞曞紑鏄剧ず鍐呴儴宸ュ簭 */
                <div key={group.id}>
                  {/* Component header row 鈥?楂樺害浠?GANTT_ROW_H.GROUP 鍙栧€硷紝涓庡乏渚х墿鐞嗛攣姝?*/}
                  <div
                  ...
                  {group.isExpanded !== false && (
                    <>
                      ...
                      {/* Placeholder row for "+鏂板鍩虹宸ュ簭" button alignment */}
                      {/* 楂樺害浠?GANTT_ROW_H.SPAWNER 鍙栧€硷紝涓庡乏渚?BlackboardSpawner 鎶樺彔鎬佺墿鐞嗛攣姝?*/}
                      <div
                        className="box-border border-b border-slate-800/20"
      ...
      {/* Phase 21 鈥?Component Deletion Modal */}
...
function ComponentTreeList(props: ComponentTreeListProps) {
  ...
  const isMacro = viewMode === "MACRO"
  const groupRowHeight = isMacro ? GANTT_ROW_H.MACRO_GROUP : GANTT_ROW_H.GROUP
  ...
  return (
    <ul className="text-[11px]">
      {components.map((group, idx) => {
        ...
        // Phase 15 - 根据 index 分配主题色
        const themeColor = COMPONENT_PALETTE[idx % COMPONENT_PALETTE.length]

        return (
          <li key={group.id}>
            {/* Component Group Header 鈥?楂樺害浠?GANTT_ROW_H.GROUP 鍙栧€硷紝涓庡彸渚ч潰鏉跨墿鐞嗛攣姝?*/}
            <div
              style={{ height: groupRowHeight }}
              className={`box-border px-3 hover:bg-slate-800/50 transition-all cursor-pointer border-b border-slate-800/30 border-l-3 ${
                groupOverdue 
              ...
              {isMacro ? (
                <div className="flex h-full flex-col justify-center gap-1.5 py-2">
                  <div className="flex items-start gap-2 min-w-0">
                    <button
                      type="button"
                      className="w-5 h-5 flex items-center justify-center rounded text-slate-700 cursor-default shrink-0"
                      disabled
                    >
                      ▸
                    </button>
                    ...
                    />
                    <span className={`min-w-0 flex-1 text-[14px] leading-4 break-all ${groupOverdue ? "text-emerald-400" : groupCompleted ? "text-emerald-400" : themeColor.base}`}>
                      {group.name}
                    </span>
                  </div>
...
/* ========================== ComponentTrack (MACRO) 鈥?Phase 20: Entity Slider ========================== */
...
function ComponentTrack({ group, timelineStart, dayWidth, role, milestones, colorIndex }: ComponentTrackProps) {
  ...
  if (group.tasks.length === 0) {
    return <div className="border-b border-slate-800/20" style={{ height: rowHeight }} />
  }
  ...
  return (
    <div
      className="box-border flex items-center border-b border-slate-800/20 relative hover:bg-slate-900/20 transition-colors"
...
// Phase 20 鈥?MilestoneLine: thin dashed yellow, no glow
...
function TaskTrack({ node, indexInParent, timelineStart, dayWidth, viewMode, role, milestones }: TaskTrackProps) {
  ...
  return (
    <div
      className="box-border flex items-center border-b border-slate-800/15 relative hover:bg-slate-900/10 transition-colors"
      style={{ height: GANTT_ROW_H.TASK }}
      data-task-id={node.id}
    >
      <div
