    ...
    <div className="relative flex h-full w-full flex-1">
          ...
          return (
            <>
              ...
              <text
                ...
                dominantBaseline="middle"
                className="fill-red-700/80 text-[11px] font-mono font-medium"
              >
                USL: {usl.toFixed(2)}
              </text>
              ...
              <text
                ...
                dominantBaseline="middle"
                className="fill-cyan-600/90 text-[11px] font-mono font-medium"
              >
                LSL: {lsl.toFixed(2)}
              </text>
              ...
              {data.map((d, i) => {
                ...
                return (
                  <g key={i}>
                    ...
                    {/* Lower Whisker */}
                    <line x1={x} y1={padding.top + yScaleLocal(d.q1)} x2={x} y2={padding.top + yScaleLocal(d.min)} stroke="#71717a" strokeWidth="1" />
                    <line x1={x - whiskerCapWidth / 2} y1={padding.top + yScaleLocal(d.min)} x2={x + whiskerCapWidth / 2} y2={padding.top + yScaleLocal(d.min)} stroke="#71717a" strokeWidth="1" />
                    ...
                    {labelDisplay !== "none" && (
                      <text
                        ...
                        style={{ fill: '#10b981', textShadow: '0 0 4px #050505, 0 0 4px #050505, 0 0 6px #050505' }}
                      >
                    ...
                    {d.outliers.map((outlier, oi) => {
                        ...
                        const clampedOutlierY = Math.max(padding.top + 4, Math.min(padding.top + chartHeight - 4, outlierY));
                        const isVisible = outlierY >= padding.top - 20 && outlierY <= padding.top + chartHeight + 20;
                        if (!isVisible) return null;
                        const dotColor = highlightOutliers
                          ? (aboveUSL ? "#dc2626" : belowLSL ? "#0891b2" : "#d97706")
                          : "#52525b";
                        const textColorClass = highlightOutliers
                          ? (aboveUSL ? "fill-red-600/80" : belowLSL ? "fill-cyan-600/80" : "fill-amber-600/80")
                          : "fill-zinc-600";
                        return (
                          <g key={`outlier-${i}-${oi}`}>
                            <circle cx={x} cy={clampedOutlierY} r="2.5" fill={dotColor} />
                            <text x={x + 8} y={clampedOutlierY} dy="3" textAnchor="start" className={`text-[11px] font-mono font-medium ${textColorClass}`}>
                              {outlier.toFixed(2)}
                            </text>
                          </g>
                        );
                    ...
                    {/* X-axis labels */}
                    <text x={x} y={padding.top + chartHeight + 16} textAnchor="middle" className="fill-zinc-300 text-[10px] font-mono font-medium">
                      {d.label}
                    </text>
              ...
              {/* Y-axis label */}
              <text x={16} y={padding.top + chartHeight / 2} textAnchor="middle" transform={`rotate(-90, 16, ${padding.top + chartHeight / 2})`} className="fill-zinc-500 text-[10px]">
                测量值 / Value
              </text>
      ...
      {tooltip.visible && tooltip.data && (
        <div
          ...
          style={{ left: Math.min(tooltip.x + 12, 780), top: Math.max(tooltip.y - 110, 10) }}
        >
