...
import { CalendarDays, CircleDot, ClipboardCheck, ImageIcon, ShieldCheck, User, Zap } from "lucide-react";

import { Badge } from "@/components/ui/badge";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Separator } from "@/components/ui/separator";
import { StatusPill } from "./StatusPill";
import { IK_ENERGY_MAP, type IKTestData } from "./ik-test-data";

export function IkDashboard({ data, pdfUrl }: { data: IKTestData; pdfUrl?: string | null }) {
        ...
        <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-sm text-muted-foreground">
              型号 {target.model} · 试验部位 {target.testPart} · 等级 {target.selectedIKLevel}
            </p>
          </div>
          <div className="flex items-center gap-3">
            <Badge
              variant={isPass ? "default" : "destructive"}
              className={isPass ? "h-9 bg-success px-4 text-sm text-success-foreground" : "h-9 px-4 text-sm"}
            >
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
              ...
              <CardContent className="flex flex-col gap-2 pt-4">
                {IK_ENERGY_MAP.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <span className={active ? "text-sm font-semibold text-primary" : "text-sm text-muted-foreground"}>
                        {item.level}
                      </span>
                      <span className={active ? "font-mono text-sm font-semibold text-primary" : "font-mono text-sm text-muted-foreground"}>
          ...
          <Card className="lg:col-span-1">
            <CardHeader className="border-b">
              ...
              <CardDescription>图2 · 未勾选项保留为原始状态</CardDescription>
            </CardHeader>
            <CardContent className="flex flex-col gap-3 pt-4">
              <StatusPill label="是否损坏" value={target.status.isDamaged} />
              <StatusPill label="是否影响防水" value={target.status.affectsWaterproof} />
              <StatusPill label="是否影响绝缘" value={target.status.affectsInsulation} />
              <StatusPill label="是否影响功能" value={target.status.affectsFunction} />
          ...
          <div className="flex flex-col gap-6">
            <Card>
              ...
              <CardContent className="flex flex-col gap-3 pt-4">
                <SignRow role="试验人" name={result.tester} date={result.testDate} />
                <SignRow role="审核人" name={result.reviewer} date={result.reviewDate} />
              </CardContent>
            </Card>
                ...
                <div className="flex flex-col gap-3 rounded-lg border border-border p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <p className="text-xs leading-relaxed text-muted-foreground">{appendix.caption}</p>
                    </div>
                  </div>
                  {pdfUrl ? (
                    <a
...
function MetricCard({
  ...
  return (
    <Card className={highlight ? "border-primary/40 bg-primary/5" : success ? "border-success/40 bg-success/5" : undefined}>
      <CardContent className="flex flex-col gap-2 py-1">
        <div className="flex items-center gap-2 text-muted-foreground">
          <span className={highlight ? "text-primary" : success ? "text-success" : "text-muted-foreground"}>{icon}</span>
          <span className="text-sm">{label}</span>
        </div>
        <div className={highlight ? "text-2xl font-semibold text-primary" : success ? "text-2xl font-semibold text-success" : "text-2xl font-semibold"}>
          {value}
        </div>
...
function SignRow({ role, name, date }: { role: string; name: string; date: string }) {
  return (
    <div className="flex items-center justify-between rounded-lg border border-border px-4 py-3">
