...
const statusValueSchema = z
  .boolean()
  .nullable()
  .describe("勾选为 true，明确未勾选为 false，无法判断或漏勾选为 null");
...
export const IK_ENERGY_MAP = Object.entries(IK_ENERGY_VALUES).map(([level, joules]) => ({
  level,
  joules,
}));
...
export function validateEnergyMapping(data: IKTestData): IKTestData {
  ...
  if (expected !== undefined && data.energy.impactEnergyJ !== expected) {
    throw new Error(`${data.target.selectedIKLevel} 应对应 ${expected.toFixed(1)} J，解析结果不一致`);
  }
