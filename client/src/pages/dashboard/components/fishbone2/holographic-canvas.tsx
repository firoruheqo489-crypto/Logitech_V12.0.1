...
export function HolographicCanvas({
  ...
  return (
    <div className="relative h-full w-full" onClick={onResetView} role="presentation">
      <div
        aria-hidden
      ...
      <div className="absolute inset-0 flex items-center justify-center p-2 sm:p-6">
        <div className="relative w-full" style={{ aspectRatio: `${VIEW.w} / ${VIEW.h}`, maxHeight: "100%" }}>
          <Wires matrix={matrix} selectedId={selectedId} activeDimension={activeDimension} />
          <Overlay
            matrix={matrix}
            target={target}
  ...
  return (
    ...
    >
      ...
      <line
        ...
        style={{ opacity: activeDimension ? 0.4 : 1, transition: "opacity 0.5s ease" }}
      />
      <line
        x1={GEO.SPINE_START}
        y1={GEO.SPINE_Y}
        ...
        return (
          ...
          >
            ...
            {causes.map((cause, i) => {
              ...
              return (
                <g key={cause.id}>
                  <line x1={a.x} y1={a.y} x2={l.x} y2={l.y} stroke={dotColor} strokeWidth={1} strokeDasharray="1.5 2.5" opacity={0.7} />
                  <circle
                    cx={a.x}
                    cy={a.y}
                    r={isSel ? 4.5 : 3}
                    ...
                    style={
                      isSel || isActive
                        ? undefined
                        : { transformOrigin: `${a.x}px ${a.y}px`, animation: `pulse-node ${2 + i * 0.4}s ease-in-out infinite` }
                    }
                  />
                  {isSel && <circle cx={a.x} cy={a.y} r={9} fill="none" stroke={dotColor} strokeWidth={0.75} opacity={0.6} />}
                </g>
              );
            })}
          </g>
    ...
    <div className="absolute inset-0">
      {CATEGORY_META.map((cat) => {
        ...
        return (
          <div
            key={cat.id}
            className={`absolute inset-0 transition-all duration-500 ${isMuted ? "z-0 opacity-10 grayscale" : isActive ? "z-20 opacity-100" : "z-10 opacity-100"}`}
            style={{ pointerEvents: "none" }}
          >
            <div className="absolute -translate-x-1/2 -translate-y-1/2 select-none text-center transition-all duration-500" style={{ left: px(g.end.x), top: py(g.end.y) }}>
              <div className={`font-mono text-[9px] uppercase tracking-[0.32em] transition-colors duration-500 ${isActive ? "text-cyan-300" : "text-cyan-300/60"}`}>{cat.code}</div>
              <div
                className={`font-sans text-base font-medium tracking-wide transition-colors duration-500 ${isActive ? "text-cyan-50" : "text-cyan-50/90"}`}
                style={isActive ? { textShadow: "0 0 15px rgba(0,243,255,0.8)" } : undefined}
              >
                {cat.label}
              </div>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: isSel ? "color-mix(in oklch, var(--cyan) 8%, transparent)" : "rgba(255,255,255,0.02)",
                ...
                >
                  <span
                    ...
                    style={{ color: vMeta.color, textShadow: vMeta.glow ? `0 0 8px ${vMeta.color}` : undefined }}
                  >
                    <span>[{vMeta.sigil}] {vMeta.label}</span>
                    <span className="inline-block size-1 rounded-full" style={{ backgroundColor: meta.color }} />
                  </span>
                  <span className={`font-sans text-[12px] leading-tight tracking-wide transition-colors ${vMeta.strike ? "line-through decoration-1" : ""} ${isSel ? "text-cyan-50" : `${vMeta.text} group-hover:text-cyan-50`}`}>
                    {cause.label}
                  </span>
                </button>
              );
      ...
      <TargetReticle target={target} />
    </div>
  );
}

function TargetReticle({ target }: { target: TargetData }) {
  return (
    <div className="absolute -translate-x-1/2 -translate-y-1/2" style={{ left: px(GEO.TARGET_X), top: py(GEO.SPINE_Y) }}>
      <div className="relative flex items-center justify-center">
        <div aria-hidden className="absolute size-[150px] rounded-full border border-dashed border-red-500/20" style={{ animation: "spin-slow 24s linear infinite" }} />
        <div aria-hidden className="absolute size-[120px] rounded-full border border-red-500/15" style={{ animation: "spin-slow 16s linear infinite reverse" }} />
        ...
        <div
          ...
          style={{
            ...
            boxShadow: "0 0 50px -12px oklch(0.6 0.2 22 / 0.6), inset 0 0 24px -10px oklch(0.7 0.2 22 / 0.5)",
          }}
        >
          <div
            aria-hidden
            ...
            style={{
              background: "linear-gradient(to bottom, oklch(0.7 0.2 22 / 0.35), transparent)",
          ...
          <div className="mt-1 whitespace-nowrap font-sans text-lg font-semibold tracking-wide text-red-50">{target.label}</div>
