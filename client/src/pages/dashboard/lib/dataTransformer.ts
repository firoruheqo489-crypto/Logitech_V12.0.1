 ...
 */
export function transformProjectToData(project: DashboardProject, index: number): ProjectData {
  return {
    no: String(index + 1),
    identity: {
    ...
    milestones: {
      ...
      t1SizeQualified: project.t1DimensionOk || '-',
    ...
    uploadBatch: project.createdAt ? new Date(project.createdAt).toISOString() : undefined,
  };
}
...
export function transformDataToProject(data: ProjectData): Record<string, string | undefined> {
  return {
    ...
    riskLevel: data.identity.riskLevel === '-' ? undefined : data.identity.riskLevel,
    ...
    t1DimensionOk: data.milestones.t1SizeQualified === '-' ? undefined : data.milestones.t1SizeQualified,
    ...
    currentNode: data.milestones.currentNode === '-' ? undefined : data.milestones.currentNode,
