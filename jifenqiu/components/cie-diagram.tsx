...
import { chromaticity } from "@/lib/report-data"
import { glassPanel, subTitle } from "@/lib/ui"

// CIE 1931 光谱轨迹 (spectral locus) 标准坐标 x,y
...
function locusPath() {
  const pts = LOCUS.map(([x, y], i) => `${i === 0 ? "M" : "L"}${px(x).toFixed(1)},${py(y).toFixed(1)}`)
  return pts.join(" ") + " Z" // 闭合 line of purples
}

// 精密坐标读数：黑色硬件子格，标签灰、数值青
...
export function CieDiagram() {
  const pointX = px(Number(chromaticity.x))
  const pointY = py(Number(chromaticity.y))
  const sdcmPass = chromaticity.sdcm <= chromaticity.sdcmTarget
      ...
      <div className="flex items-center justify-center">
        <svg viewBox={`-10 -10 ${W + 20} ${H + 20}`} className="h-56 w-auto" role="img" aria-label="CIE 1931 色品坐标图">
          ...
          {[0.2, 0.4, 0.6, 0.8].map((g) => (
            <g key={g}>
              <line x1={px(g)} y1={0} x2={px(g)} y2={H} stroke="rgba(255,255,255,0.04)" />
              <line x1={0} y1={py(g)} x2={W} y2={py(g)} stroke="rgba(255,255,255,0.04)" />
            </g>
          ))}
      ...
      <div className="mt-3 flex items-center justify-between rounded-lg border border-white/[0.03] bg-black/40 px-3 py-2">
        <div className="flex flex-col gap-0.5">
          ...
          <span className="font-mono text-sm font-semibold text-gray-100 tabular-nums">
            {chromaticity.sdcm.toFixed(1)} step
          </span>
        </div>
        <span
