...
function build(
  ...
  return {
    ...
    name,
    sections: {
      routine: [...commonRoutine, ...(extra.routine ?? [])].map(([n, s]) => item(n, s)),
      destructive: [...commonDestructive, ...(extra.destructive ?? [])].map(([n, s]) =>
        item(n, s),
      ),
      reliability: [...commonReliability, ...(extra.reliability ?? [])].map(([n, s]) =>
        item(n, s),
      ),
    },
  }
