...
import { modules, getModuleStats, getOverallStats, type ReportModule } from "@/lib/report-data"
...
function aggregateRisk(mod: ReportModule): {
  ...
  if (stats.untested / stats.total >= 0.5) return { label: "VERIFICATION VOID", tone: "void" }
    ...
    <section aria-labelledby="summary-heading" className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <StatCard
          ...
          suffix="%"
          hint={`${overall.modulePass} / ${overall.moduleTotal} 模块判定合格（仅 EMC）`}
          tone="fail"
        />
        <StatCard
          icon={Layers}
        ...
        <StatCard
          ...
          hint={`占全部 ${overall.total} 项的 ${Math.round((overall.untested / overall.total) * 100)}%`}
        />
        <StatCard
          icon={AlertTriangle}
          label="系统级风险源"
