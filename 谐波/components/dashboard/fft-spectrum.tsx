...
import type { HarmonicBar } from "@/lib/power-data"
...
function SpectrumTooltip({
  ...
  return (
    <div className="rounded-xl border border-white/[0.08] bg-black/80 px-3 py-2 backdrop-blur-xl">
      ...
      <p className="text-xs font-medium text-[#00F3FF]">Magnitude: {bar.magnitude}%</p>
      <p className="text-xs font-medium text-slate-400">Limit: {bar.limit}%</p>
      {bar.exceeds && (
        <p className="text-xs font-semibold text-[#FF003C]">Exceeds IEC limit</p>
      )}
    </div>
  )
}

export function FftSpectrum({ data }: { data: HarmonicBar[] }) {
  // The IEC reference line tracks roughly the 5% standard ceiling.
  const limitLine = 5

  return (
    <div className="h-[350px] w-full">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ top: 10, right: 8, left: -16, bottom: 0 }} barCategoryGap="22%">
          <defs>
          ...
          <Bar dataKey="magnitude" radius={[4, 4, 0, 0]}>
            {data.map((entry) => (
              <Cell
                key={entry.order}
                fill={entry.exceeds ? "url(#crimsonGradient)" : "url(#cyanGradient)"}
              />
            ))}
          </Bar>
        </BarChart>
