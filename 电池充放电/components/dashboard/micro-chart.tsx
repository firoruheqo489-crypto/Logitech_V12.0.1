...
import type { TimeSeriesPoint } from '@/lib/battery-data'
...
function MicroTooltip({ active, payload, label }: TooltipContentProps<number, string>) {
  ...
  return (
    <div className="rounded-lg border border-white/10 bg-black/90 p-3 font-mono text-[11px] shadow-[0_8px_32px_rgba(0,0,0,0.9)] backdrop-blur-xl">
      ...
      <p className="mt-1 text-[#E8B84B]">{'电压 '}{point?.v.toFixed(3)}{' V'}</p>
      ...
      <ResponsiveContainer width="100%" height="100%">
        <ComposedChart data={data} margin={{ top: 10, right: 10, bottom: 5, left: 0 }} className="bg-transparent">
          ...
          <YAxis
            ...
            allowDataOverflow
            stroke="rgba(255,255,255,0.1)"
            tick={{ fill: '#E8B84B', fontSize: 10, fontFamily: 'var(--font-mono)' }}
            tickLine={false}
            label={{
              value: 'V',
          ...
          <Line
            ...
            dot={false}
            isAnimationActive={false}
            style={{ filter: 'drop-shadow(0px 0px 8px rgba(232, 184, 75, 0.6))' }}
          />
        </ComposedChart>
      </ResponsiveContainer>
    </div>
