...
/** Six Sigma adjudication lifecycle for every hypothesis node */
...
export interface Cause {
  ...
  label: string
  risk: Risk
  /** confidence the cause contributes to the defect, 0-1 */
  contribution: number
  /** short telemetry note shown in the inspector */
  note: string
  metric: string
  /** initial adjudication state */
  validation: Validation
  /** verification protocol (e.g. "2-Way ANOVA on fixture dims") */
  protocol: string
  /** accountable owner */
  owner: string
}
...
export interface Category {
  ...
  label: string
  side: "top" | "bottom"
  /** column index 0..2 from the target outward */
  col: number
  causes: Cause[]
}
...
> = {
  HIGH: { label: "HIGH", color: "var(--risk-high)", dot: "bg-red-400", text: "text-red-400" },
  MED: { label: "MED", color: "var(--risk-med)", dot: "bg-amber-400", text: "text-amber-400" },
  LOW: { label: "LOW", color: "var(--risk-low)", dot: "bg-emerald-400", text: "text-emerald-400" },
}
...
export const VALIDATION_META: Record<
  Validation,
  {
    /** ascii sigil rendered in the brackets */
    sigil: string
    label: string
    en: string
    /** css color token */
    color: string
    /** tailwind text class for the node label */
    text: string
    glow: boolean
    strike: boolean
  }
...
/** editable, runtime adjudication state for a single node */
...
/** the live, mutable matrix: category id → ordered list of cause nodes */
export type MatrixData = Record<string, Cause[]>

/** category descriptor without its causes (layout + labels only) */
export type CategoryMeta = Omit<Category, "causes">

/** category metadata only (no causes) — stable, used for canvas layout & labels */
export const CATEGORY_META = CATEGORIES.map(({ causes, ...meta }) => meta)

/** build a fresh deep-copied matrix from the seed categories */
export function seedMatrix(): MatrixData {
  const m: MatrixData = {}
  for (const cat of CATEGORIES) {
    m[cat.id] = cat.causes.map((c) => ({ ...c }))
  }
  return m
}

/** defaults applied to a freshly injected hypothesis node */
...
/** attach x positions on the spine, indexed by col (0 = nearest target) */
...
/**
 * Distribute N anchor points along the rib's bezier parameter (t).
 * Works for any node count so the canvas stays in sync with live CRUD edits.
 */
...
export function ribGeometry(cat: Category, count = 3): RibGeometry {
  ...
  // gentle fiber-optic sweep: leave the spine horizontally, arc up/down to the header
  ...
  // leader stub points slightly to the left of each anchor
...
/** convert view-space coordinate to a percentage string for the HTML overlay */
