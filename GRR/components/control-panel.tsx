...
import { toleranceOf, type StudyConfig } from '@/lib/gage-rnr'
  ...
  return (
    <GlassPanel className="px-4 py-4">
      ...
      <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 lg:grid-cols-4">
        <Field label="Parts (N)" value={cfg.parts} min={2} max={20} onChange={(v) => onDims({ parts: v })} hint="2–20" />
        <Field
          label="Appraisers (K)"
          value={cfg.operators}
          min={2}
          ...
          hint="2–8"
        />
        <Field label="Trials (R)" value={cfg.trials} min={2} max={7} onChange={(v) => onDims({ trials: v })} hint="2–7" />
        <Field
          label="Alpha (α)"
          value={cfg.alpha}
          step={0.01}
